from PyQt6.QtWidgets import QVBoxLayout, QHBoxLayout, QWidget, QDialog, QTextEdit
...
from .BasicWidgets import DropDownMenu, Button
from .DynamicSettingsWidget import DynamicSettingsWidget
from utils.ParamList import ParameterList, NumParam, ChoiceParam
...
class AddonBaseClass(QDialog):
    def __init__(self, title="Undefined", on_apply=None, title_postfix=" Addon"):
        ...
        self.dataGetter = lambda: (None, None, None)
...
class AddonsMenu(DropDownMenu):
    def __init__(self, onChoose, dataGetter, addons=None):
        addons = addons or []
        for addon in addons:
...
class FindPeaksAddon(AddonBaseClass):
    ...
    def find_peaks(self):
        addon_data = self.dataGetter()
        if addon_data is None:
            print("No data")
            return
        data, plot1, plot2 = addon_data
        ...
        if data["type"] in ["FFT", "Waveform"]:
            ...
            if len(x) < 2 or len(y) < 2:
                self.textEdit.setPlainText("Not enough visible data to detect peaks")
                return

            yRange = y.max() - y.min()
            # search for peaks
            n_x_distance = max(1, int(self.settings["minDistance"] / (x[1] - x[0])))
            ...
            height_inf_line = pg.InfiniteLine(pos=height, angle=0, pen=pg.mkPen('r', width=2, style=Qt.PenStyle.DashLine))
...
class WaveformViewerWidget(QWidget):
    def __init__(self, navHeight=100, plotTypeMenu=True, scaleMenu=True, settingsBtn=True, addonsMenu=True):
        ...
        plotsHLayout.addWidget(self.plotLayoutWidget)
    ...
    def plotInfiniteLine(self, y, color='w', width=2, style=Qt.PenStyle.DashLine):
    ...
    def computePlotData(self, x, y, Ts):
        ...
        if plotType == "FFT":
            ...
            img = pg.ImageItem()
            img.setImage(Sxx)
            img.setPos(float(x.min()), float(f.min()))
            img.setTransform(tr)            

            try:
                self.histogramPlot.sigLevelChangeFinished.disconnect(self.histogramLevelsChanged)
            except TypeError:
                pass
